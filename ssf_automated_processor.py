...
class ACSDataFrame(tk.Frame):
    ...
    def open_directory(self):
        ...
        with open('new_ff.csv', 'w') as f:
            throwaway=0
        for folder in os.listdir(directory):
            if(os.path.isdir(folder)):
                with open('ssf_processed_values.csv', 'a+') as f:
                    for file in os.listdir(directory+'/'+folder):
                        if(file.endswith('.dat') and file.startswith('data')):
                            print(file)
                            self.open_file(folder+'/'+file)
                            line=self.processed_values[-1]
                            string_to_write=line[0]
                            for item in line[1:]:
                                string_to_write=string_to_write+','+str(item)
    ...
    def calculate_cn2(self):
        ...
        self.slope_struct_functions_x_60=[]
        self.slope_struct_functions_y_60=[]
        self.slope_struct_functions_x_45=[]
        self.slope_struct_functions_y_45=[]
        self.slope_struct_functions_x_30=[]
        self.slope_struct_functions_y_30=[]
        ...
        for i in range(len(self.slope_struct_functions)):
            ...
            def functional_form_full(r, r0):
                return 2*0.77*6.88/(d_sub**2)*(d_sub/r0)**(5/3)*(r**2/(1+r**2)+0.438*r**(0.2555))
            
            
            r_vals=[rbin_size*i+rbin_size/2 for i in range(len(self.slope_struct_functions[i]))]
            params, covariances=curve_fit(functional_form_full, r_vals, self.slope_struct_functions[i])
            r_0=params[0]
            self.r0_calculated.append(r_0)
            data_to_append.append(r_0)
            cn2=wavelength**2/(4*np.pi**2)*(r_0/3)**(-5/3)/path_length
            data_to_append.append(cn2)
            self.cn2_calculated.append(cn2)

            params, covariances=curve_fit(functional_form_xy, r_vals, self.slope_struct_functions_x[i])
            r_0x=params[0]
            params, covariances=curve_fit(functional_form_xy, r_vals, self.slope_struct_functions_y[i])
            r_0y=params[0]
            r_0xy=np.mean([r_0x,r_0y])
            data_to_append.append(r_0xy)
            cn2xy=wavelength**2/(4*np.pi**2)*(r_0xy/3)**(-5/3)/path_length
            data_to_append.append(cn2xy)
            ...
            if(not self.process_data):
                break
        self.processed_values.append(data_to_append)
    ...
    def process_data_method(self):
        ...
        if(self.process_data):
            ...
            self.controller.frames[ACSSlopeStructPlotFrame].set_slope_struct_functions(self.slope_struct_functions, self.slope_struct_functions_x, self.slope_struct_functions_y, self.r0_calculated)
...
class ACSSlopeStructPlotFrame(tk.Frame):
    ...
    def set_slope_struct_functions(self, slope_struct_funcs, slope_struct_funcs_x, slope_struct_funcs_y, r0_vals):
    ...
    def plot_data(self):
        ...
        D_form=[2*0.77*6.88/(d_sub**2)*(d_sub/(self.r0[self.r0_framenum]))**(5/3)*(r**2/(1+r**2)+0.438*r**(0.2555)) for r in r_form]
        
        self.x=self.a.plot(rs, self.slope_struct_functions[self.framenum], label=r'$D_s$')
        self.x=self.a.plot(rs, self.slope_struct_functions_x[self.framenum], label=r'$D_s^x$')
        self.x=self.a.plot(rs, self.slope_struct_functions_y[self.framenum], label=r'$D_s^y$')
